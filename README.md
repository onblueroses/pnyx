...
**What that number measures.** Training labels are generated by `google/gemini-2.0-flash-001` (`scripts/02_label_texts.py`), and evaluation runs against a held-out slice of that same labeled set. So F1 0.974 measures how closely the distilled DeBERTa model reproduces the labeling model, not agreement with human judgments of argument quality. No human-annotated evaluation set exists in this repo; `scripts/validate_labels.py` checks label distributions and degenerate patterns, not correctness.
